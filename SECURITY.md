...
Pavilo 当前默认仍设计用于**可信网络环境**（内网、VPN 或配置了适当访问控制的代理后）。最新稳定版是 v1.6.0。值班台人员治理管的是临时席位、举报和墓碑，不是会员名录；宿主 JWT 是可选接入端口。嵌入预览只对配置的宿主来源放开 iframe。不发布接入 SDK，接入方式见 [接入说明](docs/integrate.md)。
...
- **v1.6**：嵌入预览。已发布。浏览器检查改为阻塞发布。不是 SDK。
- **下一次发布**：一段话接入。不发布 SDK。通用 Policy 端口、视觉审核和提交前词表更晚，而且词表不是接入的门槛。
